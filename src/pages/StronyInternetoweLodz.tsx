            ...
            <div className="flex flex-wrap justify-center gap-3 max-w-4xl mx-auto">
              {cityLinks.map((city, index) => (
                <motion.div key={city.name} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }}>
                  <Link to={city.href} className={`px-5 py-2.5 rounded-full border transition-all ${city.name === "Łódź" ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border/50 hover:border-primary/50 hover:bg-primary/5"}`}>{city.name}</Link>
                </motion.div>
              ))}
            </div>
          </div>
